...
- `KartRider.exe -profile:launcher` 실행과 레지스트리에 등록된 준비 인스턴스 검색

합성 PIN의 v1→v2 변환·멱등성·백업 정리와 일부 fail-closed guard는 자동
검증됩니다. 성공한 중단 복구나 rollback은 자동 검증되지 않습니다. 실제 원본 PIN,
WinForms UI, 레지스트리, 관리자 권한 mutex 해제와 게임 실행을 연결한 현재 배포본의
end-to-end 테스트도 아직 없습니다.
...
- 라이선스 진행도 0~3단계, 즉 L2까지 여섯 개 completion mask 저장
...
frame codec, 제한된 로그인 profile 파싱, JSON 기본 round-trip과 서버 lifecycle은
자동 검증됩니다. 라이선스 UI, 전체 장착 필드와 실제 UDP 초기화 순서는 자동 테스트
대상이 아닙니다.
...
개발 중 수동 확인했습니다. 서버가 아이템을 추첨하거나 효과를 직접 시뮬레이션하지
않으므로 모든 아이템의 동일 확률을 서버가 보장하지 않으며, 보유 아이템과 사용
요청의 일치도 강제하지 않습니다.
...
현재 자동 테스트는 총 19개 case입니다.

- 접속기 7개: 합성 format-v1 PIN의 flags `0/1/2/3` 변환, 헤더·암호 키 보존,
  endpoint·저장 루트 갱신, 멱등성·백업 정리와 접근 불가 프로세스 fail-closed
- 서버 12개: clear/encrypted frame round-trip, IV 진행, 잘못된 checksum 거부,
  제한된 username 파싱, JSON reopen, loopback 기본값, IPv6 거부, TCP/UDP 시작·종료,
  동시 start/stop, disconnect cleanup과 플래그전 전용 아이템 GameSlot relay
...
Ghost, Mine 두 형태와 Rollingbomb envelope가 byte 단위로 상대에게만 전달되는지와
잘못된 base type 차단을 검사합니다. 실제 게임 클라이언트의 item 효과, observer,
...
- 라이선스 completion mask와 L2 UI의 실제 재접속 유지
...
- self-contained 게시, 비-Windows 서버 실행과 첫 공개 push 이후 GitHub Actions
...
현재 근거만으로 P236 게임 서버의 미구현 항목이라고 단정하지 않습니다. L1/Pro는
이 클라이언트 자체의 완성된 라이선스 단계가 아니므로 현재 서버 지원 범위도 L2까지입니다.

## 공개 전 권장 검증 순서

1. `scripts/Build.ps1`로 source-boundary, 빌드와 19개 테스트를 실행합니다.
2. `scripts/Publish.ps1`로 깨끗한 게시 폴더를 만듭니다.
