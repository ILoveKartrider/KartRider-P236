...
- 신규·기존 프로필은 `level=4`, masks floor `[31,7,31,63,30,0]`을 적용해
  `0x41~0x44`를 완료 처리하고 `0x45`까지 열되 `0x46`은 잠금 상태로 유지
- 접속기는 알려진 원본 P236 코드와 vtable을 검증한 뒤 Challenge3 기반 L1 호환 훅을
  적용하며, `0x41`의 `devil0`/`devil1` 대마왕 효과, `0x44`의 native Challenge3
  화살표 생성·추적, `0x45`의 Factory obstacle/dummy 초기화와 종료 정리를 보강
- 접속기의 `실행 시 L1 호환 훅 적용` 체크박스로 인스턴스별 적용 여부를 저장하며,
  해제한 실행에서는 PIN/XML·username·mutex 처리를 유지하고 L1 훅만 생략
- 접속기의 별도 `L1 데이터 패치` 탭과 `L1 패치 적용…` 버튼은 사용자가 고른 donor
  원본 RHO와 P236 Data를 입력으로
  기존 RHO 12개를 missing-only 병합하고 트랙 RHO 22개를 로컬 복사하며, P236 호환
  L1 BML과 대상 `aaa.pk`를 생성; 저장소·실행 파일에는 게임 데이터 payload를 포함하지 않음
- 34개 RHO와 `aaa.pk`를 하나의 복구 가능한 트랜잭션으로 교체하고, 최초 원본 백업,
  SHA-256 외부 변경 감지, 중단 복구, 완전 무변경 재적용과 `원본 복원` 지원
- item 확률 편집기와 공통 Data 변경 잠금을 사용하고, 관리 중인 L1 설치 위에 item
  metadata를 덮어쓰는 순서를 거부해 `aaa.pk`와 두 백업의 불일치를 예방
...
필요합니다. `0x45`의 Factory 기믹 로드는 실제 클라이언트에서 확인했습니다.
`0x41` 대마왕 훅은 1-frame 충돌 이벤트를 native base update 직후 읽도록 구현했습니다.
`0x44`는 P236의 절대 route/segment 값을 후대 Challenge4의 `disappearArrowIdx`로
오인하지 않도록 커스텀 visibility override를 비활성화했습니다. 화살표 생성·표시·추적은
native Challenge3 상태 머신에 맡기며 실제 `0x44` 미션에서 표시·추적을 확인했습니다.
checkpoint 10 이후 숨김은 미구현입니다.
`0x42` 로두마니 일당 견제는 **미구현**입니다. 원본 데이터와 미션 설명상 이 미션은
디지니와 같은 팀으로 로두마니·투투를 상대하고, 제한시간 안에 디지니가 1위가 되도록
해야 하는 팀 아이템전입니다. 현재 P236 Challenge3 호환 경로는 AI 한 명만 생성하고
로컬 라이더가 AI보다 먼저 완주했는지만 판정하므로 원본 팀 구성과 승리 조건을 재현하지
못합니다. 호환 RHO의 진입과 단일 `rocket` 큐브만 부분 동작하며, 완료 판정 지원으로
간주하지 않습니다. `0x44`는 Challenge4용 `arrow` 대신 Challenge3가 화살표 객체를
만드는 `showArrow="true"`를 사용합니다.
...
- rider 채팅을 초기화된 같은 방 rider·observer에게 중계하고 발신자 echo는 중복 방지를 위해 제외
...
현재 자동 테스트는 총 90개 case입니다.

- 접속기 23개: 합성 format-v1 PIN의 flags `0/1/2/3` 변환, 헤더·암호 키 보존,
  BOM 없는 레거시 XML과 원본 PIN/XML endpoint 불일치의 최초 준비, endpoint·저장 루트
  갱신, 멱등성·백업 정리, 반쪽 커밋 복구, portable 상대 경로 이동·중복 우선순위·
  손상 catalog fallback, 인스턴스별 L1 훅 설정의 구버전 기본값·저장 유지와 자동 검색
  기본값과 비활성화 시 패치 호출 생략, 접근 불가 프로세스 fail-closed, L1 호환 x86
...
- 클라이언트 데이터 38개: 아이템 확률 JSON round-trip·검증, 다섯 확률표 import/apply,
  전체 RHO/aaa semantic 보존, KRData 암호화 mode/key 보존, 원본 pair·외부 변경
  fail-closed, 원자적 백업·멱등성·중단 복구; 합성 donor/P236 RHO로 L1 34개
  archive 생성, Challenge3 호환 변환, metadata 검증, 완전 무변경 재적용과 원본 복원
...
UDP echo/time-sync packet과 접속기의 실제 UI 상호작용·mutex·registry·process launch는 자동 테스트에서
실행하지 않습니다.
...
- 접속기의 새 UI로 donor를 선택하는 흐름과 패처가 깨끗한 P236 복사본에서 생성한
  RHO를 사용한 실제 게임 실행. 변환 결과는 642개 엔트리 경계와 metadata까지 로컬
  통합 검증했지만 생성 직후 별도 클라이언트 인수 실행은 남아 있음
...
- L1 `0x42`는 원본의 디지니 아군·로두마니/투투 상대 팀 구성과 디지니 1위 판정을
  구현하지 않았습니다. 진입과 단일 `rocket` 지급만 부분 동작합니다.
...
클라이언트에는 완전한 L1 UI·트랙 자산이 없어 로컬 L1 데이터 패처가 필요합니다.
기존 수동 변환본의 L1 UI·주행은 확인했지만 패처가 새로 생성한 깨끗한 복사본의 별도
end-to-end 실행은 남아 있습니다. PRO(level 5)는 미지원·미검증입니다.

## 공개 전 권장 검증 순서

1. `scripts/Build.ps1`로 source-boundary, 빌드와 90개 테스트를 실행합니다.
