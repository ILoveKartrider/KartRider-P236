...
src/
  ...
  KartRider.P236.ItemProbabilities/ P236 아이템 확률·L1 로컬 Data 변환
...
접속기의 별도 `L1 데이터 패치` 탭에 있는 `L1 패치 적용…` 버튼은 완성된 변형
RHO를 내려받거나 내장하지 않습니다.
사용자가 직접 선택한 L1 donor 클라이언트의 `Data`와 현재 P236 `Data`를 동시에 읽어,
다음 결과를 로컬에서 생성합니다.

- 기존 P236 RHO 12개에는 donor의 필요한 항목만 병합하고 P236 항목 충돌은 유지
- L1 미션과 타임어택에 필요한 donor 트랙 RHO 22개를 로컬 설치본 사이에서 복사
- donor Challenge4 정의를 그대로 이식하지 않고 검증된 P236 Challenge3 호환
  `0x41~0x46` 정의와 구형 설명 UI를 실행 시 생성
- 생성된 34개 RHO의 실제 key, data hash와 크기에 맞춰 대상 `aaa.pk`만 갱신
- 지원 donor의 theme 추가 571개와 P236 우선 충돌 7개를 의미 단위로 검증하고,
  RHO 엔트리와 `aaa.pk` track 목록을 정상 클라이언트와 같은 순서로 정렬

패처는 필수 donor 파일 36개의 SHA-256을 내장된 지원 리비전 지문과 모두
대조합니다. 파일 하나라도 지문이 다르면 바이트 단위가 다른 재패키징·변형본으로
판단하고 대상 `Data`를 변경하기 전에 적용을 거부합니다.

사용 순서는 다음과 같습니다.

1. 같은 `Data`를 사용하는 모든 클라이언트를 종료합니다.
2. 아이템 확률도 바꿀 예정이면 먼저 서버 런처에서 해당 설정을 적용합니다.
3. 접속기에서 패치 대상 P236 인스턴스를 선택합니다.
4. `L1 데이터 패치` 탭에서 `L1 패치 적용…`을 누르고 본인이 적법하게 보유한 지원
   donor의 클라이언트 루트 또는 `Data` 폴더를 선택합니다.
5. 생성·재개방 검증과 전체 교체가 끝난 뒤 `실행 시 L1 호환 훅 적용`을 켜고 실행합니다.

L1 설치와 아이템 확률 적용은 모두 `aaa.pk`를 수정하므로 같은 Data 변경 잠금을
공유합니다. L1이 설치된 상태에서 아이템 확률 적용은 원본 백업의 의미가 달라지는 것을
막기 위해 거부됩니다. 이 경우 접속기에서 `원본 복원` → 아이템 확률 적용 →
`L1 패치 적용…` 순서로 진행합니다.

패처는 처음 적용할 때 대상의 원래 파일 존재 여부와 SHA-256을
`Data/.p236-l1-data-backup-v1`에 한 번만 보존합니다. 같은 레시피 재적용은 파일을
바꾸지 않으며, `원본 복원`은 설치 후 외부 변경이 없는지 검증한 뒤 기존 12개 RHO와
`aaa.pk`를 되돌리고 새로 추가했던 22개 트랙 RHO를 제거합니다. 중간 종료 시에는
복구 마커를 이용해 전체 파일 집합을 이전 상태로 되돌립니다.

추출·변환된 폴더는 donor 입력으로 지원하지 않습니다. 패처가 원본 RHO를 직접 읽어
DDS/PNG 등 엔트리 이름과 저장 속성을 보존해야 합니다. 저장소와 릴리스에는 donor나
생성된 RHO/BML/텍스처 등의 게임 데이터 페이로드가 포함되지 않으며, 검증용 SHA-256
지문과 로컬 변환 규칙만 포함됩니다.
...
2. PIN/XML의 서버 endpoint와 문서 저장 루트를 트랜잭션으로 갱신하며, 검증에 실패하면
   두 파일을 백업에서 복구합니다.
3. `Profile/launcher.xml`에 사용자명을 기록합니다.
4. 선택한 인스턴스와 일치하는 mutex만 해제한 뒤 게임을 실행합니다.
5. `실행 시 L1 호환 훅 적용`이 선택되어 있으면 알려진 원본 P236 코드와 vtable을
   다시 검증한 뒤 L1 미션용 Challenge3 런타임 호환 훅을 적용합니다. 검증·적용에
   실패하면 시작한 게임을 종료합니다.

이 체크박스는 인스턴스별로 저장되며 기존 설정에는 기본적으로 켜집니다. 해제하면 다음
실행부터 L1 훅만 건너뛰고 PIN/XML, username과 mutex 처리는 그대로 수행합니다. 이미
실행 중인 클라이언트에서 적용된 훅을 제거하는 기능은 아닙니다.

L1 `0x42` 로두마니 일당 견제는 **미구현**입니다. 원본 미션은 디지니와 한 팀으로
로두마니·투투를 상대해 디지니가 1위가 되어야 하지만, P236 Challenge3 호환 경로는
AI 한 명과 로컬 라이더 선착만 판정합니다. 현재는 미션 진입과 단일 `rocket` 큐브만
부분 동작하며 원본 팀 구성·아이템 팀전·승리 조건은 지원하지 않습니다.
